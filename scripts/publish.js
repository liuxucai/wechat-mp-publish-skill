...
async function main() {
  ...
  // 1. 先导航到微信首页，确保从已登录会话中拿到 token（避免读到无关 tab 的 URL 误判未登录）
  lib.writeStatus('导航到微信首页以取得 token...');
  console.log('导航到微信首页...');
  await lib.navigate(ws, 'https://mp.weixin.qq.com/');

  // 2. 检查登录状态
  lib.writeStatus('检查登录状态...');
  const token = await lib.getToken(ws);
  if (!token) {
    console.error('❌ 未登录！请先在浏览器扫码登录微信公众平台，再重新运行本脚本');
